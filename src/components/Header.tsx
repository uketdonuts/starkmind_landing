  ...
  return (
    <header className="bg-slate-900/95 backdrop-blur-sm text-white sticky top-0 z-50 border-b border-slate-700/50">
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        <a href="/" className="flex items-center space-x-3 hover:opacity-80 transition-all duration-150 group" aria-label="Ir al inicio">
          <div className="relative">
            <img src="/img/logo.svg" alt="StarkMind Logo" className="w-10 h-10 group-hover:scale-105 transition-transform duration-150" />
          </div>
        ...
        <div className="hidden lg:flex items-center space-x-8">
          ...
          <div className="flex items-center space-x-3 ml-6 border-l border-slate-700 pl-6">
            <span className={`text-xs font-medium transition-all duration-300 ${currentLang === 'es' ? 'text-white font-semibold' : 'text-slate-400 hover:text-slate-300'}`}>ES</span>
            <button 
              onClick={toggleLanguage}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
            ...
            <span className={`text-xs font-medium transition-all duration-300 ${currentLang === 'en' ? 'text-white font-semibold' : 'text-slate-400 hover:text-slate-300'}`}>EN</span>
          </div>
        </div>
        ...
        <div className="lg:hidden flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            <span className={`text-xs font-medium transition-all duration-300 ${currentLang === 'es' ? 'text-white font-semibold' : 'text-slate-400'}`}>ES</span>
            <button 
              onClick={toggleLanguage}
              className={`relative inline-flex h-5 w-9 items-center rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
                currentLang === 'en' ? 'bg-slate-600' : 'bg-slate-700'
              }`}
            >
              <span className={`inline-block h-3 w-3 transform rounded-full bg-white shadow-sm transition-all duration-300 ${
                currentLang === 'en' ? 'translate-x-5' : 'translate-x-1'
              }`}></span>
            </button>
            <span className={`text-xs font-medium transition-all duration-300 ${currentLang === 'en' ? 'text-white font-semibold' : 'text-slate-400'}`}>EN</span>
          </div>

          {/* Hamburger Button */}
          <button
            onClick={toggleMobileMenu}
            className="inline-flex items-center justify-center p-2 rounded-md text-slate-300 hover:text-white hover:bg-slate-800 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
            aria-expanded="false"
